...
class UserState:
    ...
    # произвольный комментарий от пользователя
    comment: str = ""
    peer_id: int = 0
...
def build_slots_keyboard(page: int, selected: Set[Any], slots: List[Any]) -> VkKeyboard:
    ...
    if not slots:
        # Datasource не вернул смен — оставляем только возможность перейти к комментарию
        kb.add_callback_button("Далее ▶", color=VkKeyboardColor.PRIMARY,
...
class Bot:
    ...
    def _handle_comment(self, user_id: int, peer_id: int, text: str) -> None:
        ...
        for event in self.long_poll.listen():
            try:
                if event.type == VkBotEventType.MESSAGE_NEW:
                    ...
                    if text.lower() == "/start":
                        self.handle_start(user_id, peer_id)
                    elif get_state(user_id).step == Step.COMMENT:
                        # Пользователь вводит комментарий — любой текст кроме /start
                        self._handle_comment(user_id, peer_id, text)
...
if __name__ == "__main__":
    ...
    Bot(GROUP_TOKEN, GROUP_ID, datasource).run()
